...
@property (nonatomic, strong) UIView *blackFullScreen;
...
- (UIView *)spinnerView
{
    if (!_spinnerView) {
        ...
        [[UIView alloc] initWithFrame:CGRectMake(0, 0, // Will be centered in window later
                                                 sizeForSpinnerView.width,
                                                 sizeForSpinnerView.height)];
...
- (void)presentWithAnimationTypes:(SPAnimationTypes)animationTypes
{
    ...
    NSTimeInterval duration = animationTypes == SPAnimationTypeNone ? 0.0 : kSPIntroAnimationLength;
...
- (void)dismiss
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:kSPOutroAnimationLength animations:^{
